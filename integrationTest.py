...
BASELINE_DATATEST_PATH = DECOMPILER_DIR + "decompile_baseline_datatest"
...
def run_datatest(test_case, sample, plugin=True, datatest_path=DATATEST_PATH, plugin_path=None):
    """
    Run a single data test, with optional plugin
    """
...
class T000BuildBaselineDecompiler(unittest.TestCase):
    """
    Build the Ghidra baseline decompiler executables and exercise representative data tests
    """
    ...
    def test_01_ghidra(self):
        ...
        for f in (DECOMPILER_PATH, BASELINE_DATATEST_PATH):
            if os.path.exists(f):
                command = f"rm -f {f}"
                ...
                self.assertEqual(0, result.returncode,
                    "unable to clean previous decompiler executable files.  Is it write-protected?")

        # build the decompiler executable used by Ghidra
        command = "bazel build -c opt @ghidra_baseline//:decompile"
        logger.info(f"Running {command}")
        result = subprocess.run(command, check=True, capture_output=True,
                                shell=True, encoding="utf8")
        self.assertEqual(0, result.returncode,
        ...
        self.assertEqual(result, DECOMPILER_PATH,
                         "Unable to install the baseline decompiler executable")

        # build the decompiler datatest executable
        command = "bazel build -c dbg @ghidra_baseline//:decompile_datatest"
        logger.info(f"Running {command}")
        result = subprocess.run(command, check=True, capture_output=True,
                                shell=True, encoding="utf8")
        self.assertEqual(0, result.returncode,
            "bazel build of the Ghidra decompiler data_test failed")
        result = shutil.copy(BAZEL_BUILD_DATATEST_PATH, BASELINE_DATATEST_PATH)
        self.assertEqual(result, BASELINE_DATATEST_PATH,
                         "Unable to install the decompiler datatest_baseline executable")

    def test_02_shorter_exemplars(self):
        """
...
class T001BuildPluginEnabledDecompiler(unittest.TestCase):
    """
    Build the Ghidra plugin-enabled decompiler executables and exercise representative data tests
    """
    ...
    def test_01_ghidra(self):
        ...
        # build the decompiler executable used by Ghidra
        command = "bazel build -c opt @ghidra_patched//:decompile"
        logger.info(f"Running {command}")
        result = subprocess.run(command, check=True, capture_output=True,
                                shell=True, encoding="utf8")
        self.assertEqual(0, result.returncode,
        ...
        result = shutil.copy(BAZEL_BUILD_DECOMPILER_PATH, DECOMPILER_PATH)
        self.assertEqual(result, DECOMPILER_PATH,
                         "Unable to install the patched decompiler executable")

        # build the decompiler datatest executable
        command = "bazel build -c dbg @ghidra_patched//:decompile_datatest"
        logger.info(f"Running {command}")
        result = subprocess.run(command, check=True, capture_output=True,
                                shell=True, encoding="utf8")
        self.assertEqual(0, result.returncode,
        ...
        self.assertEqual(result, DATATEST_PATH,
                         "Unable to install the patched decompiler datatest executable")
...
class T100Thixotropist(unittest.TestCase):
    """
    Run thixtropist's research initiative tests
    """
    ...
    def test_01_install_patched_decompile_datatest(self):
        """
        Build and install the patched datatest framework
        """
        logger.info(f"Cleaning the executable directory {DECOMPILER_DIR}")
        for f in (DECOMPILER_PATH, DATATEST_PATH):
            if os.path.exists(f):
       ...
       # build the decompiler executable used by Ghidra
        ...
        result = subprocess.run(command, check=True, capture_output=True,
                                shell=True, encoding="utf8")
        self.assertEqual(0, result.returncode,
            "bazel build of the patched Ghidra decompiler failed")
        result = shutil.copy(BAZEL_BUILD_DECOMPILER_PATH, DECOMPILER_PATH)
        self.assertEqual(result, DECOMPILER_PATH,
                         "Unable to install the patched decompiler executable")
        # build the decompiler datatest executable
    ...
    def test_02_build_riscv_plugin(self):
        """
        Build the current riscv_plugin using source and BUILD files under
        the 'plugins' directory
        """
        logger.info("Removing any previous plugin")
        command = f"rm -f {self.PLUGIN_PATH}"
        logger.info(f"Running {command}")
        result = subprocess.run(command, check=True, capture_output=True,
                                shell=True, encoding="utf8")
        self.assertEqual(0, result.returncode,
            "unable to clean previous decompiler plugin")
        logger.info("Building the riscv_vector plugin")
        command = "bazel build -c dbg @//plugins:riscv_vector"
        logger.info(f"Running {command}")
        result = subprocess.run(command, check=True, capture_output=True,
                                shell=True, encoding="utf8")
        self.assertEqual(0, result.returncode,
            "bazel build of the RISC-V plugin failed")
        logger.info(f"copying {self.PLUGIN_NAME} into the plugin search directory /tmp")
        result = shutil.copy(f"bazel-bin/plugins/{self.PLUGIN_NAME}", "/tmp")
        self.assertEqual(result, self.PLUGIN_PATH,
                         "Unable to install the decompiler plugin")

    def test_03_shorter_exemplars(self):
        """
        Verify correct behavior under valgrind with smaller binaries,
        generally save files of less than 10 KB
        """
